---
name: local-coding
description: 编程任务优先走本地 LM Studio 代码模型（默认 Qwen2.5-Coder-14B），仅当用户明确"不限时长"时才用 qwen3-coder-30b-a3b-instruct；复杂决策、跨文件重构、架构设计仍交主模型
...
| "不限时长"任务（用户显式声明）                                      | 本地        | qwen3-coder-30b-a3b-instruct |

**判断依据**：

- 只看少量文件、或答案可以一眼验证 → 本地
- 需要看 3+ 个文件、或需要设计决策 → 主模型
- 用户说"慢慢来 / 不赶时间 / 尽量好 / 不限时长" → 本地 qwen3-coder-30b-a3b-instruct
...
| `coder-deep` / `coder-long` / `deep-coder` | qwen3-coder-30b-a3b-instruct | ~18GB / Q4_K_M | 需要 CPU Offload，速度 ~9-22 tok/s，质量更高，仅用户声明时用 |
...
# 深度模式（qwen3-coder-30b-a3b-instruct），仅在用户声明不限时长时
...
- ❌ 同一轮任务内不要并发请求本地 coder 模型（router 的切换锁只在加载阶段持锁，转发阶段并发会误卸载在用模型）
